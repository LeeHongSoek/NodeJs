...
const employeesInfo = require('../rt_info/ifEmployees')
...
router.use('/:employeeNumber', (req, res) => {
    ...
    if (req.method === 'GET') {
        pool.connect.query(employeesInfo.selectSqlOne, [req.params.employeeNumber], (err, result, fields) => {
            if(err) {
    ...
    if (req.method === 'DELETE') {
        pool.connect.query(deleteSqlOne, [req.params.employeeNumber], (err,rows, fields) => {
            if(err) {
                console.log(`sql에 에러 발생 : ${err}`)
